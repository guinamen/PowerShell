...
#$events = Get-EventLog -LogName System -Source "Service Control Manager" | ? {$_.Message -like "The Apache Tomcat for BI 4 service entered the*" -and $_.TimeGenerated -ge $startingDate }
...
$percentUpTime = "{0:n2}" -f ( ($UpTimeMinutes/$minutesInPeriod)*100)
